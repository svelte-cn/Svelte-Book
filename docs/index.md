---
layout: home

title: Svelte 教程 2026 | Svelte 5 完全指南 | 从入门到精通
description: 最全 Svelte 教程！涵盖 Svelte 5 Runes 响应式系统、21 章节完整教程 + 3 个实战项目。零基础入门到精通！

head:
  - tag: title
    content: Svelte 教程 2026 | Svelte 5 完全指南 | 从入门到精通
  - tag: meta
    attrs:
      property: og:title
      content: Svelte 教程 2026 | Svelte 5 完全指南
  - tag: meta
    attrs:
      property: og:description
      content: 最全 Svelte 教程！涵盖 Svelte 5 Runes 响应式系统、21 章节完整教程 + 3 个实战项目

hero:
  name: "Svelte 教程"
  text: "从入门到精通"
  tagline: "2026 最新版 | Svelte 5 新特性 | 21 章节完整教程 | 3 个实战项目"
  actions:
    - theme: brand
      text: 🚀 开始阅读
      link: /chapters/part1/ch01-introduction/
    - theme: alt
      text: 📖 在线阅读
      link: /chapters/part1/ch01-introduction/
    - theme: alt
      text: ⭐ GitHub Stars
      link: https://github.com/svelte-cn/Svelte-Book
    - theme: alt
      text: 📦 源码下载
      link: https://github.com/svelte-cn/Svelte-Book/archive/refs/heads/main.zip

features:
  - title: ⭐ 最全 Svelte 教程
    details: 21 章节完整内容，从环境搭建到项目部署，涵盖 Svelte/SvelteKit 所有核心知识点
  - title: 🚀 Svelte 5 新特性
    details: 详解 Runes 响应式系统、Snippets、$props 等 Svelte 5 全新特性
  - title: 💻 3 个实战项目
    details: Todo 待办应用、博客系统、电商前台，完整源码可直接使用
  - title: ⚡ 性能优化与部署
    details: 包含测试策略、CI/CD 部署、SEO 优化等生产级最佳实践

footer: MIT License | Copyright © 2026 Nix_____（码徒）
---
